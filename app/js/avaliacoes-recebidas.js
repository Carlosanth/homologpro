    ...
    <div class="no-print" style="display:flex; justify-content:flex-end; gap:8px; margin-top:16px">
      ${!av.semServico && (sit === 'reprovado' || sit === 'parcial') ? (
        ...
        <div style="margin-right:auto; display:flex; align-items:center; gap:12px; flex-wrap:wrap">
          ...
          ${d.notifAvaliacaoModo === 'aprovacao' && !av.notificadoEm ? `<button class="btn btn-primary btn-sm" onclick="aprovarEnviarNotificacaoAutomatica('${av.id}')" style="display:inline-flex; align-items:center; gap:6px">${ic('check', 13)}Aprovar e enviar (HTML)</button>` : ''}
        </div>
      `) : ''}
